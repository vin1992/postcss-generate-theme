/*
 * @Author: your name
 * @Date: 2021-07-19 21:54:53
 * @LastEditTime: 2021-07-19 22:55:19
...
const verifyDeclareProp = (decl) => {
  return (
    compositionProps.includes(decl.prop) ||
    decl.prop.split("-").pop() === "color"
  );
};
...
const saveFaultColors = (color) => {
  faultColors.push(color);
  writeFile("faultColor.log", faultColors.join(","), "utf8", (err) => {
    if (err) throw err;
    console.log("异常色值已收集!");
  });
};
...
const findModeColor = (lightVal, mode, decl) => {
  ...
  if (
    ["background", "background-color"].includes(decl.prop) &&
    lightVal === "#fff"
  ) {
    key = `${key}_bg`;
  }
  ...
  try {
    let [darkVal, nightVal] = theme[key].value || [];
    if (mode === "dark") {
      return darkVal;
    } else if (mode === "night") {
      return nightVal;
...
const transpileGradient = (decl, mode) => {
  ...
  if (ast.length > 0 && ast[0].type.includes("-gradient")) {
    ...
    colorList.forEach((color, id) => {
      ...
      if (color.type === "hex") {
        ...
        item = findModeColor(transformColorValToHex(color.value), mode, decl);
        ...
        item = findModeColor(lightVal, mode, decl);
...
const transpileCompositionValue = (decl, mode) => {
  ...
  if (pureValNodes.length) {
    pureValNodes.forEach((node, id) => {
      ...
      if (isHexColor(node) || isColorKeyWords(node.value)) {
        item = findModeColor(transformColorValToHex(node.value), mode, decl);
...
const processCssValue = (decl, mode) => {
  ...
  if (ignoreColor.includes(decl.value)) {
    ...
    // 单个颜色属性
    return findModeColor(transformColorValToHex(decl.value), mode, decl);
